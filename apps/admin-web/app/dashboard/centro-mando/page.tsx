...
export default function CentroMandoPage() {
  ...
  const [liveAlert, setLiveAlert] = useState<TacticalSocketIncident | null>(null);

  useEffect(() => {
    if (!session) return;
    if (!hasPermission(session.permissions, SITOP_PERMISSIONS.DASHBOARD_VIEW)) {
  ...
  return (
    <div className="space-y-6">
      <header className="rounded-xl border border-slate-800 bg-slate-900/50 px-6 py-5">
        ...
        <h1 className="mt-2 text-lg font-semibold text-slate-100">Centro de Mando</h1>
        <p className="mt-1 text-sm text-slate-400">
          Mapa táctico, incidentes en vivo y despacho operativo.
        </p>
      </header>

      <TacticalAlertBanner alert={liveAlert} onDismiss={() => setLiveAlert(null)} />
      ...
      {canViewIncidents && (
        <>
          <ExecutiveSummary incidents={incidents} session={session} loading={listLoading} />
